
package com.company.json;

import com.AllValutes.*;
import com.google.gson.annotations.Expose;
import com.google.gson.annotations.SerializedName;


public class Valute_ {

    @SerializedName("AUD")
    @Expose
    private AUD aUD;
    @SerializedName("AZN")
    @Expose
    private AZN aZN;
    @SerializedName("GBP")
    @Expose
    private GBP gBP;
    @SerializedName("AMD")
    @Expose
    private AMD aMD;
    @SerializedName("BYN")
    @Expose
    private BYN bYN;
    @SerializedName("BGN")
    @Expose
    private BGN bGN;
    @SerializedName("BRL")
    @Expose
    private BRL bRL;
    @SerializedName("HUF")
    @Expose
    private HUF hUF;
    @SerializedName("HKD")
    @Expose
    private HKD hKD;
    @SerializedName("DKK")
    @Expose
    private DKK dKK;
    @SerializedName("USD")
    @Expose
    private USD uSD;
    @SerializedName("EUR")
    @Expose
    private EUR eUR;
    @SerializedName("INR")
    @Expose
    private INR iNR;
    @SerializedName("KZT")
    @Expose
    private KZT kZT;
    @SerializedName("CAD")
    @Expose
    private CAD cAD;
    @SerializedName("KGS")
    @Expose
    private KGS kGS;
    @SerializedName("CNY")
    @Expose
    private CNY cNY;
    @SerializedName("MDL")
    @Expose
    private MDL mDL;
    @SerializedName("NOK")
    @Expose
    private NOK nOK;
    @SerializedName("PLN")
    @Expose
    private PLN pLN;
    @SerializedName("RON")
    @Expose
    private RON rON;
    @SerializedName("XDR")
    @Expose
    private XDR xDR;
    @SerializedName("SGD")
    @Expose
    private SGD sGD;
    @SerializedName("TJS")
    @Expose
    private TJS tJS;
    @SerializedName("TRY")
    @Expose
    private TRY tRY;
    @SerializedName("TMT")
    @Expose
    private TMT tMT;
    @SerializedName("UZS")
    @Expose
    private UZS uZS;
    @SerializedName("UAH")
    @Expose
    private UAH uAH;
    @SerializedName("CZK")
    @Expose
    private CZK cZK;
    @SerializedName("SEK")
    @Expose
    private SEK sEK;
    @SerializedName("CHF")
    @Expose
    private CHF cHF;
    @SerializedName("ZAR")
    @Expose
    private ZAR zAR;
    @SerializedName("KRW")
    @Expose
    private KRW kRW;
    @SerializedName("JPY")
    @Expose
    private JPY jPY;

    /**
     * No args constructor for use in serialization
     *
     */
    public Valute_() {
    }

    /**
     * 
     * @param hKD
     * @param eUR
     * @param dKK
     * @param uSD
     * @param cAD
     * @param xDR
     * @param bGN
     * @param nOK
     * @param rON
     * @param sGD
     * @param aZN
     * @param cZK
     * @param kGS
     * @param sEK
     * @param tMT
     * @param bRL
     * @param uAH
     * @param cHF
     * @param kZT
     * @param zAR
     * @param iNR
     * @param cNY
     * @param uZS
     * @param aUD
     * @param kRW
     * @param jPY
     * @param pLN
     * @param gBP
     * @param mDL
     * @param bYN
     * @param hUF
     * @param aMD
     * @param tRY
     * @param tJS
     */
    public Valute_(AUD aUD, AZN aZN, GBP gBP, AMD aMD, BYN bYN, BGN bGN, BRL bRL, HUF hUF, HKD hKD, DKK dKK, USD uSD, EUR eUR, INR iNR, KZT kZT, CAD cAD, KGS kGS, CNY cNY, MDL mDL, NOK nOK, PLN pLN, RON rON, XDR xDR, SGD sGD, TJS tJS, TRY tRY, TMT tMT, UZS uZS, UAH uAH, CZK cZK, SEK sEK, CHF cHF, ZAR zAR, KRW kRW, JPY jPY) {
        super();
        this.aUD = aUD;
        this.aZN = aZN;
        this.gBP = gBP;
        this.aMD = aMD;
        this.bYN = bYN;
        this.bGN = bGN;
        this.bRL = bRL;
        this.hUF = hUF;
        this.hKD = hKD;
        this.dKK = dKK;
        this.uSD = uSD;
        this.eUR = eUR;
        this.iNR = iNR;
        this.kZT = kZT;
        this.cAD = cAD;
        this.kGS = kGS;
        this.cNY = cNY;
        this.mDL = mDL;
        this.nOK = nOK;
        this.pLN = pLN;
        this.rON = rON;
        this.xDR = xDR;
        this.sGD = sGD;
        this.tJS = tJS;
        this.tRY = tRY;
        this.tMT = tMT;
        this.uZS = uZS;
        this.uAH = uAH;
        this.cZK = cZK;
        this.sEK = sEK;
        this.cHF = cHF;
        this.zAR = zAR;
        this.kRW = kRW;
        this.jPY = jPY;
    }

    public AUD getAUD() {
        return aUD;
    }

    public void setAUD(AUD aUD) {
        this.aUD = aUD;
    }

    public AZN getAZN() {
        return aZN;
    }

    public void setAZN(AZN aZN) {
        this.aZN = aZN;
    }

    public GBP getGBP() {
        return gBP;
    }

    public void setGBP(GBP gBP) {
        this.gBP = gBP;
    }

    public AMD getAMD() {
        return aMD;
    }

    public void setAMD(AMD aMD) {
        this.aMD = aMD;
    }

    public BYN getBYN() {
        return bYN;
    }

    public void setBYN(BYN bYN) {
        this.bYN = bYN;
    }

    public BGN getBGN() {
        return bGN;
    }

    public void setBGN(BGN bGN) {
        this.bGN = bGN;
    }

    public BRL getBRL() {
        return bRL;
    }

    public void setBRL(BRL bRL) {
        this.bRL = bRL;
    }

    public HUF getHUF() {
        return hUF;
    }

    public void setHUF(HUF hUF) {
        this.hUF = hUF;
    }

    public HKD getHKD() {
        return hKD;
    }

    public void setHKD(HKD hKD) {
        this.hKD = hKD;
    }

    public DKK getDKK() {
        return dKK;
    }

    public void setDKK(DKK dKK) {
        this.dKK = dKK;
    }

    public USD getUSD() {
        return uSD;
    }

    public void setUSD(USD uSD) {
        this.uSD = uSD;
    }

    public EUR getEUR() {
        return eUR;
    }

    public void setEUR(EUR eUR) {
        this.eUR = eUR;
    }

    public INR getINR() {
        return iNR;
    }

    public void setINR(INR iNR) {
        this.iNR = iNR;
    }

    public KZT getKZT() {
        return kZT;
    }

    public void setKZT(KZT kZT) {
        this.kZT = kZT;
    }

    public CAD getCAD() {
        return cAD;
    }

    public void setCAD(CAD cAD) {
        this.cAD = cAD;
    }

    public KGS getKGS() {
        return kGS;
    }

    public void setKGS(KGS kGS) {
        this.kGS = kGS;
    }

    public CNY getCNY() {
        return cNY;
    }

    public void setCNY(CNY cNY) {
        this.cNY = cNY;
    }

    public MDL getMDL() {
        return mDL;
    }

    public void setMDL(MDL mDL) {
        this.mDL = mDL;
    }

    public NOK getNOK() {
        return nOK;
    }

    public void setNOK(NOK nOK) {
        this.nOK = nOK;
    }

    public PLN getPLN() {
        return pLN;
    }

    public void setPLN(PLN pLN) {
        this.pLN = pLN;
    }

    public RON getRON() {
        return rON;
    }

    public void setRON(RON rON) {
        this.rON = rON;
    }

    public XDR getXDR() {
        return xDR;
    }

    public void setXDR(XDR xDR) {
        this.xDR = xDR;
    }

    public SGD getSGD() {
        return sGD;
    }

    public void setSGD(SGD sGD) {
        this.sGD = sGD;
    }

    public TJS getTJS() {
        return tJS;
    }

    public void setTJS(TJS tJS) {
        this.tJS = tJS;
    }

    public TRY getTRY() {
        return tRY;
    }

    public void setTRY(TRY tRY) {
        this.tRY = tRY;
    }

    public TMT getTMT() {
        return tMT;
    }

    public void setTMT(TMT tMT) {
        this.tMT = tMT;
    }

    public UZS getUZS() {
        return uZS;
    }

    public void setUZS(UZS uZS) {
        this.uZS = uZS;
    }

    public UAH getUAH() {
        return uAH;
    }

    public void setUAH(UAH uAH) {
        this.uAH = uAH;
    }

    public CZK getCZK() {
        return cZK;
    }

    public void setCZK(CZK cZK) {
        this.cZK = cZK;
    }

    public SEK getSEK() {
        return sEK;
    }

    public void setSEK(SEK sEK) {
        this.sEK = sEK;
    }

    public CHF getCHF() {
        return cHF;
    }

    public void setCHF(CHF cHF) {
        this.cHF = cHF;
    }

    public ZAR getZAR() {
        return zAR;
    }

    public void setZAR(ZAR zAR) {
        this.zAR = zAR;
    }

    public KRW getKRW() {
        return kRW;
    }

    public void setKRW(KRW kRW) {
        this.kRW = kRW;
    }

    public JPY getJPY() {
        return jPY;
    }

    public void setJPY(JPY jPY) {
        this.jPY = jPY;
    }

    @Override
    public String toString() {
        return "Valute_{" +
                "aUD=" + aUD +
                ", aZN=" + aZN +
                ", gBP=" + gBP +
                ", aMD=" + aMD +
                ", bYN=" + bYN +
                ", bGN=" + bGN +
                ", bRL=" + bRL +
                ", hUF=" + hUF +
                ", hKD=" + hKD +
                ", dKK=" + dKK +
                ", uSD=" + uSD +
                ", eUR=" + eUR +
                ", iNR=" + iNR +
                ", kZT=" + kZT +
                ", cAD=" + cAD +
                ", kGS=" + kGS +
                ", cNY=" + cNY +
                ", mDL=" + mDL +
                ", nOK=" + nOK +
                ", pLN=" + pLN +
                ", rON=" + rON +
                ", xDR=" + xDR +
                ", sGD=" + sGD +
                ", tJS=" + tJS +
                ", tRY=" + tRY +
                ", tMT=" + tMT +
                ", uZS=" + uZS +
                ", uAH=" + uAH +
                ", cZK=" + cZK +
                ", sEK=" + sEK +
                ", cHF=" + cHF +
                ", zAR=" + zAR +
                ", kRW=" + kRW +
                ", jPY=" + jPY +
                '}';
    }
}
